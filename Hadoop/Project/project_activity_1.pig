-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/rachit/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/rachit/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/rachit/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/rachit/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/rachit/outputs' USING PigStorage('\t');
